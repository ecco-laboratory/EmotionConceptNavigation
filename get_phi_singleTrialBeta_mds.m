...
[project_dir, fmri_data_dir, ~, ~, ~, ~, ~, ~, spm_timing_dir, motion_regressors_dir, nuisance_regressors_dir, spm1stlevel_dir, ~, phi_dir, subjects] = set_up_dirs_constants();
...
if mds_aligned
    ...
    phi_dir_voxelwise = fullfile(phi_dir, 'univariate', ['singleTrialBeta_voxelwise_mdsAligned', mds_metric], ['seed', seed], ICA_type); 
    phi_dir_regionwise = fullfile(phi_dir, 'univariate', ['singleTrialBeta_regionwise_mdsAligned', mds_metric], ['seed', seed], ICA_type);
else
    theta_dir = fullfile(project_dir, 'data', 'beh', ['theta_mds_', mds_metric], ['seed', seed]);
    phi_dir_voxelwise = fullfile(phi_dir, 'univariate', ['singleTrialBeta_voxelwise_mds', mds_metric], ['seed', seed], ICA_type); 
    phi_dir_regionwise = fullfile(phi_dir, 'univariate', ['singleTrialBeta_regionwise_mds', mds_metric], ['seed', seed], ICA_type);
end
...
region_names = {'HC', 'ERC', 'vmPFC'};
region_masks = {fullfile(project_dir, 'masks', 'HC_Julich.nii'),...
...
cross_validations = {'xModalityRun', 'xModality', 'xRun'}; %{'avgAllRuns'}
phi_averaging_methods = 'component_average';%'circular_mean';
