...
if mds_aligned
    ...
    base_output_dir = fullfile(project_dir, 'outputs', 'singleTrialBetaAnalysis', ICA_type, 'incl_all_subs_trials',['onoffGridcontrast_multivariate_mdsAligned', mds_metric], ['seed', seed]);
    ...
    base_output_dir = fullfile(project_dir, 'outputs', 'singleTrialBetaAnalysis', ICA_type, 'incl_all_subs_trials',['onoffGridcontrast_multivariate_mds', mds_metric], ['seed', seed]);
...
if_rotate_phi = false; %true or false
rotate_phi_degree = 30;
if if_rotate_phi, base_output_dir = fullfile(base_output_dir, ['phi_rotate', num2str(rotate_phi_degree)]); end
        ...
        for s = 1:length(subjects)
            ...
            for r = 1:length(region_names)
                ...
                for cv = 1:length(cross_validations)
                    ...
                    elseif strcmp(current_cross_validation, 'xModalityRun_own')
                        training_idx = {{'face1', 'word1'}, {'face1', 'word2'}, {'face2', 'word1'}, {'face2', 'word2'}};
                        test_idx = {{'face1', 'word1'}, {'face1', 'word2'}, {'face2', 'word1'}, {'face2', 'word2'}};
                    elseif strcmp(current_cross_validation, 'xModality_own')
                        training_idx = {{'face1','face2'}, {'word1', 'word2'}};
                    ...
                    for t_idx = 1:length(training_idx)
                        ...
                        if strcmp(current_cross_validation, 'phi0')
                            ...
                            elseif strcmp(current_cross_validation, 'xModalityRun_own') || strcmp(current_cross_validation, 'xModality_own')
                                cv_name_to_use = strrep(current_cross_validation, '_own', '');
                                phi_value = phiRadDivByPeriod_struct.crossValidations.(cv_name_to_use).(strjoin(current_training_idx, '_'));
                            else
                                phi_value = phiRadDivByPeriod_struct.crossValidations.(current_cross_validation).(strjoin(current_training_idx, '_'));
                            end
                        end
                        if if_rotate_phi
                            period_rad = 2*pi / current_periodicity;
                            rotation_rad = deg2rad(rotate_phi_degree);
                            phi_value = mod(phi_value + rotation_rad, period_rad);
                        end
