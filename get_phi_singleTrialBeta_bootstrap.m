...
% and also average single trial betas across 
...
phi_dir_regionwise = fullfile(phi_dir, 'univariate', 'singleTrialBeta_regionwise_bootstrap', ICA_type);

non_norm_theta_trials = 'includeNonNorm';%'excludeNonNorm';%'';
angle_sources = {'Subavg'};
periodicity = {6};
runs = [1, 2]; modalities = {'word', 'face'};
brain_atlas = load_atlas('canlab2018');
smooth = 'unsmoothed'; phi_dir_regionwise = fullfile(phi_dir_regionwise, smooth); spm1stlevel_dir = fullfile(spm1stlevel_dir, smooth);


region_names = {'OFC2016ConstantinescuR5'};
region_masks = {fullfile(project_dir, 'masks', 'OFC_2016Constantinescu_r5.nii')};

                
modality_to_use = 'both_modalities';
training_ids = {{'face1', 'word1'}, {'face1', 'word2'}, {'face2', 'word1'}, {'face2', 'word2'},...
              {'face1','face2'}, {'word1', 'word2'},...
              {'face1'}, {'face2'}, {'word1'}, {'word2'}};
...
% Subset when indices are in the Slurm script
if exist('subject_idx', 'var')
    subjects = subjects(subject_idx);
end

if exist('training_idx', 'var')
    training_ids = training_ids(training_idx);
end
    ...
    for t = 1:length(angle_sources)
        ...
        for s = 1:length(subjects)
            ...
            for region_idx = 1:length(region_names)
                ...
                for t_idx = 1:length(training_ids)
                    current_training_idx = training_ids{t_idx}; 
                    ...
                    curr_phi_boot = bootstrap_phi(X_train, Y_train, current_periodicity, num_groups, num_boot, seed);

                    phi_file_name = ['boot_PhiRadDivByPeriod_', region_names{region_idx}, '_', strjoin(current_training_idx, '_'), '_', num2str(num_groups), 'groups.csv'];
                    writematrix(curr_phi_boot, fullfile(path_to_save, phi_file_name));
...
function curr_phi_boot = bootstrap_phi(X_train, Y_train, current_periodicity, num_groups, num_boot, seed)
    if nargin < 6, seed = 1; end
    rng(seed);
    n_trials = size(X_train, 1);
    n_voxels = size(Y_train, 2);

    % Split trials into contiguous groups
    group_edges = round(linspace(0, n_trials, num_groups + 1));
    group_trial_indices = cell(num_groups, 1);

    for g = 1:num_groups
        group_trial_indices{g} = (group_edges(g) + 1 : group_edges(g + 1))';
    end

    curr_phi_boot = nan(num_boot, 1);

    for boot_idx = 1:num_boot

        % Sample contiguous groups with replacement
        sampled_groups = randi(num_groups, num_groups, 1);
        boot_trial_idx = [];
        for g = 1:num_groups
            boot_trial_idx = [boot_trial_idx; group_trial_indices{sampled_groups(g)}];
        end

        X_boot = X_train(boot_trial_idx, :);
        Y_boot = Y_train(boot_trial_idx, :);
        X_train_noPhi = [cos(current_periodicity * deg2rad(X_boot)), sin(current_periodicity * deg2rad(X_boot))];
        X_design_noPhi = [ones(size(X_train_noPhi, 1), 1), X_train_noPhi];

        beta_cos_allvoxels = nan(n_voxels, 1);
        beta_sin_allvoxels = nan(n_voxels, 1);
        ...
        curr_phi_boot(boot_idx) = atan2(mean(beta_sin_allvoxels), mean(beta_cos_allvoxels)) / current_periodicity;
    end
